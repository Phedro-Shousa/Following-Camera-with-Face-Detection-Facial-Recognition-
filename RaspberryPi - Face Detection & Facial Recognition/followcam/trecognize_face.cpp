...
void trecognize_face::run(void *)
{
    ...
    for(i = 1; i <= 30; i++)
    {
        ...
        //cv :: resize(img_to_recize, img_recized, Size(SIZE_OF_FRAMES_WIDTH,SIZE_OF_FRAMES_LENGHT), 1.0, 1.0, INTER_CUBIC);
        images_of_dataset.pop_back();
        images_of_dataset.push_back(img_to_recize);
        j++;
    }
    i =1;
    for(i=1; i<=30; i++ ){ //no inicio do programa se não forem acrescentados imagens ao dataset o inidice estará a 0
       labels.push_back(i);
    }
    ...
    while(1){
        ...
        if(m_followcam->data_tdetect_face.vec_of_faces.size()){
            for(unsigned int i = 0 ; i < m_followcam->data_tdetect_face.vec_of_faces.size(); i++)
            {
                ...
                if(predicted_confidence <= 70){
                    // Create rectange to attach the image
                    //rectangle(recognized_frame_recized ,m_followcam->data_tdetect_face.vec_of_faces[i], CV_RGB(0, 255,0), 1);
        ...
        //cv::resize(recognized_frame_recized, frame_w_recognition, Size(SIZE_OF_FRAMES_WIDTH, SIZE_OF_FRAMES_LENGHT), 1.0, 1.0, INTER_CUBIC);
        //m_followcam->updater.newimagerecord(QImage(recognized_frame_recized.data, recognized_frame_recized.cols, recognized_frame_recized.rows, recognized_frame_recized.step, QImage::Format_RGB888));
