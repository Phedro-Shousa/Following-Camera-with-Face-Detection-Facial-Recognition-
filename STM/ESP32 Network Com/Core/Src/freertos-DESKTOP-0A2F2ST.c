...
/* USER CODE BEGIN Variables */
	...
	xSemaphoreHandle Sem_update_interface = 0;
...
void MX_FREERTOS_Init(void) {
  ...
  /* add semaphores, ... */
		...
		vSemaphoreCreateBinary(Sem_update_interface);
  ...
  osThreadDef(defaultTask, vTaskDataAnalyse, osPriorityNormal, 0, 128);
  defaultTaskHandle = osThreadCreate(osThread(defaultTask), NULL);

  /* definition and creation of myTask02 */
  osThreadDef(myTask02, vTaskReadSonar, osPriorityHigh, 0, 128);
  myTask02Handle = osThreadCreate(osThread(myTask02), NULL);

  /* USER CODE BEGIN RTOS_THREADS */
  /* add threads, ... */
  osThreadDef(Taskatuationservo, vTaskServoManage, osPriorityNormal, 0, 128);
  vTaskServoManageHandle = osThreadCreate(osThread(Taskatuationservo), NULL);
 
	osThreadDef(Taskreadsonar2, vTaskReadSonar2, osPriorityHigh, 0, 128);
  vTaskReadSonar2Handle = osThreadCreate(osThread(Taskreadsonar2), NULL);
	
	osThreadDef(TaskComunication, vTaskWirelessComunication, osPriorityAboveNormal, 0, 128);
  ...
  for(;;)
  {
		...
		switch(acquired_data.mode){
			...
			case SONAR:
				...
				break;
			case ADAPT_CONTROL:
				if((xQueueReceive(Q_sonar1_reading_handle, &str_reading_sonar1 , 1000)) && (xQueueReceive(Q_sonar2_reading_handle, &str_reading_sonar2 , 1000))){
  ...
  for(;;)
  {
   ...
   //code thread
		...
		if((str_reading_sonar1.sonar_dist < 400) && (str_reading_sonar1.sonar_dist > 1)){
			if(acquired_data.mode == ADAPT_CONTROL){
			//send Queue to analyse distances
				if(! xQueueSend(Q_sonar1_reading_handle,&str_reading_sonar1,1000)){
					xSemaphoreTake(uartMutexHandle, portMAX_DELAY);
					WI_FI=0;
...
void vTaskWirelessComunication(void const * argument)
{
  ...
  {
   //code thread
		if(xSemaphoreTake(Sem_update_interface, portMAX_DELAY)){
...
void read_distance1(int ID){
		...
		{
			if(icFlag_1) break;
		}while((HAL_GetTick() - startTick1) < 250);  //250ms
		icFlag_1 = 0;
		...
		xSemaphoreGive(Sem_update_interface);
		}
...
void read_distance2(int ID){
		...
		{
			if(icFlag_2) break;
		}while((HAL_GetTick() - startTick2) < 250);  //250ms
		icFlag_2 = 0;
